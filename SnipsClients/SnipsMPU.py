...
class SnipsMPU(object):
    def __init__(self, mqtt_addr, relay, sht31):
    ...
    def __extract_value(self, slots):
        result = []
        if slots is not None:
            for slot in slots:
                if slot.confidenceScore > self.__THRESHOLD_SLTO_CONFCORE_TAKE:
                    result.append(slot.slot_value.value.value)
        return result
